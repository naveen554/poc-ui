...
function SummaryCard({
  ...
  return (
    <div className="flex items-center gap-3 rounded-md border border-gray-200 bg-white px-3 py-2.5 shadow-sm">
      <span
        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded ${tone}`}
      >
        {icon}
      </span>
      <div className="min-w-0">
        ...
        <p className="text-[17px] font-semibold leading-tight text-navy-800">
          {value}
        </p>
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex rounded-full border px-2 py-0.5 text-2xs font-medium ${tone}`}
    >
      {status.replace(/_/g, " ")}
    </span>
  );
...
export function ReportsPage() {
  ...
  const contractsByBroker = useMemo(() => {
    ...
    const grouped = new Map<
      string,
      { broker: string; contracts: number; total_pgs: number }
    >();
  ...
  const policiesByProduct = useMemo(() => {
    ...
    const grouped = new Map<
      string,
      { product: string; policies: number; pgs: number }
    >();
    ...
    <div className="mx-auto flex w-full max-w-[1400px] flex-col gap-3 px-4 py-4">
      <div className="flex flex-wrap items-center gap-3 rounded-md border border-blue-200 bg-blue-50 px-3 py-2">
        <div className="flex min-w-0 flex-1 items-start gap-2">
          ...
          <p className="truncate text-[12.5px] leading-relaxed text-blue-900">
            <span className="font-semibold">Portfolio insights.</span> Filter by
            upload date to scope the executive overview.
          </p>
        </div>
        <div className="flex shrink-0 items-center gap-2 whitespace-nowrap">
          <label className="text-2xs font-medium uppercase tracking-wide text-gray-600">
            From
          </label>
          <input
            type="date"
      ...
      {overviewData && (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <SummaryCard
            ...
            hint={`${overviewData.overdue_reviews} overdue reviews`}
            ...
            <div className="grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
                  ...
                  {hasClassificationData ? (
                    ...
                    <div className="flex h-full w-full flex-col items-center justify-center gap-1 text-center">
                      ...
                      <p className="text-[11.5px] text-gray-500">
                        Try clearing the date filter or uploading contracts to
                        see the breakdown.
                      </p>
                    </div>
                  )}
              ...
              <Panel title="Portfolio Summary" bodyClassName="p-3">
                <div className="grid grid-cols-3 gap-2">
                  <MiniStat
                    label="Policies"
                    value={String(overviewData.total_policies)}
                  />
                  <MiniStat
                    label="Brokers"
                    value={String(overviewData.total_brokers)}
                  />
                  <MiniStat
                    label="Clients"
                    value={String(overviewData.total_clients)}
                    tone="text-blue-700"
                  />
                </div>
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <MiniStat
                    label="Standard PGs"
                    value={String(overviewData.standard_pgs)}
                    tone="text-emerald-700"
                  />
                  <MiniStat
                    label="Non-Standard PGs"
                    value={String(overviewData.non_standard_pgs)}
                    tone="text-amber-700"
                  />
                </div>
              </Panel>
      ...
      {tab === "contracts" && contracts && (
        <div className="flex flex-col gap-3">
            ...
            <Panel title="Validation & Risk Snapshot" bodyClassName="p-3">
              <div className="grid grid-cols-3 gap-2">
                <MiniStat
                  ...
                  value={String(
                    contracts.items.reduce(
                      (s, c) => s + c.contract_validation_failures,
                      0,
                    ),
                  )}
                  tone="text-red-600"
                ...
                <MiniStat
                  ...
                  value={String(
                    contracts.items.reduce(
                      (s, c) => s + c.contract_validation_warnings,
                      0,
                    ),
                  )}
                  tone="text-amber-600"
                ...
                <MiniStat
                  label="Uncapped"
                  value={String(
                    contracts.items.filter((c) => !c.dollar_cap).length,
                  )}
                  tone="text-navy-700"
                />
                <MiniStat
                  label="Total PGs"
                  value={String(
                    contracts.items.reduce((s, c) => s + c.total_pgs, 0),
                  )}
                />
                <MiniStat
                  label="Non-Standard"
                  value={String(
                    contracts.items.reduce((s, c) => s + c.non_standard_pgs, 0),
                  )}
                  tone="text-amber-700"
                />
                <MiniStat
                  label="Pending"
                  value={String(
                    contracts.items.reduce((s, c) => s + c.pending_pgs, 0),
                  )}
                  tone="text-blue-700"
                />
              </div>
          ...
          <Panel
            title={`Contract Portfolio (${contracts.count})`}
            bodyClassName="p-0"
          >
      ...
      {tab === "policies" && policies && (
        <div className="flex flex-col gap-3">
          ...
          <Panel
            title={`Policy Portfolio (${policies.count})`}
            bodyClassName="p-0"
          >
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left">
                ...
                <tbody>
                  {policies.items.map((p) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-[12px] text-amber-700">
                        {p.non_standard_pgs}
                      </td>
                      <td className="px-3 py-2 text-[12px] text-blue-700">
                        {p.pending_pgs}
                      </td>
                      <td className="px-3 py-2 text-[12px] text-gray-500">
          ...
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
            ...
            <Panel title="Category Snapshot" bodyClassName="p-3">
              <div className="grid grid-cols-2 gap-2">
                <MiniStat
                  label="Categories"
                  value={String(pgsByCategory.length)}
                />
                <MiniStat label="Metrics" value={String(pgs.count)} />
                <MiniStat
                  label="Attention"
                  value={String(
                    pgs.items.reduce((s, p) => s + p.attention_pgs, 0),
                  )}
                  tone="text-red-600"
                />
                <MiniStat
                  label="Amended"
                  value={String(
                    pgs.items.reduce((s, p) => s + p.amended_pgs, 0),
                  )}
                  tone="text-blue-700"
                />
              </div>
              <div className="mt-3 rounded border border-gray-100 bg-gradient-to-br from-navy-50 to-blue-50 px-3 py-2.5">
                <p className="text-2xs font-medium uppercase tracking-wide text-gray-600">
                  Avg Penalty Allocation
                </p>
                <p className="mt-0.5 text-[22px] font-bold leading-tight text-navy-800">
                  {(
                    pgs.items.reduce(
                      (s, p) => s + parseFloat(p.avg_penalty_allocation_pct),
                      0,
                    ) / (pgs.items.length || 1)
                  ).toFixed(1)}
                  %
                ...
                <p className="text-2xs text-gray-500">
                  Weighted across all PG metrics
                </p>
              </div>
            </Panel>
          </div>
                ...
                <tbody>
                  {pgs.items.map((p, idx) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-[12px] text-emerald-700">
                        {p.standard_pgs}
                      </td>
                      <td className="px-3 py-2 text-[12px] text-amber-700">
                        {p.non_standard_pgs}
                      </td>
                      <td className="px-3 py-2 text-[12px] text-red-600">
                        {p.attention_pgs}
                      </td>
      ...
      {tab === "products" && products && (
        <div className="flex flex-col gap-3">
          ...
          <Panel
            title={`Product Portfolio (${products.count})`}
            bodyClassName="p-0"
          >
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left">
      ...
      {tab === "brokers" && brokers && (
        <div className="flex flex-col gap-3">
          ...
          <Panel
            title={`Broker Portfolio (${brokers.count})`}
            bodyClassName="p-0"
          >
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left">
            ...
            <Panel title="Operations Snapshot" bodyClassName="p-3">
              <div className="grid grid-cols-2 gap-2">
                ...
                <MiniStat
                  label="Exceptions"
                  value={String(
                    operations.data.exceptions.reduce((s, e) => s + e.items, 0),
                  )}
                  tone="text-amber-700"
                />
                <MiniStat
                ...
                return (
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]">
                    <div className="flex flex-col justify-center gap-2 rounded border border-gray-100 bg-gradient-to-br from-navy-50 to-blue-50 px-4 py-3">
                      ...
                      <p className="text-[20px] font-bold leading-tight text-navy-800">
                        {day.day}
                      </p>
                      <div className="mt-1 grid grid-cols-3 gap-2">
                        <div>
                          <p className="text-2xs uppercase tracking-wide text-gray-500">
                            Contracts
                          </p>
                          <p className="text-[18px] font-semibold text-navy-800">
                            {day.contracts}
                          </p>
                        </div>
                        <div>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart
                          data={[
                            ...
                            {
                              metric: "Extracted PGs",
                              value: day.extracted_pgs,
                              fill: "#3B82F6",
                            },
                            {
                              metric: "PGs / Contract",
                              value: day.contracts
                                ? Math.round(day.extracted_pgs / day.contracts)
                                : 0,
                              fill: "#60A5FA",
                            },
                          ]}
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Line
                      yAxisId="right"
                      type="monotone"
                      dataKey={(d: { estimated_ai_cost_usd: string }) =>
                        parseFloat(d.estimated_ai_cost_usd)
                      }
                      name="AI Cost ($)"
                      stroke="#10B981"
                      strokeWidth={2}
