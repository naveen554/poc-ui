...
export function mapContractToFileDetail(contract: ContractDetails): FileDetail {
  ...
  const extractedFields: ExtractedField[] = [
    {
      label: "Client Name",
      value: contract.client_name || "Not available",
      confidence: 95,
    },
    {
      label: "Contract Date",
      value: contract.contract_date || "Not available",
      confidence: 90,
    },
    {
      label: "Agreement Period",
      value:
        contract.agreement_period_start && contract.agreement_period_end
          ? `${contract.agreement_period_start} to ${contract.agreement_period_end}`
          : "Not available",
      confidence: 95,
    },
    {
      label: "Agreement Type",
      value: contract.agreement_period_type || "Not available",
      confidence: 100,
    },
    {
      label: "Policy Numbers",
      value:
        contract.policy_numbers?.length > 0
          ? contract.policy_numbers.join(", ")
          : "None",
      confidence: 90,
    },
    {
      label: "NYL Representative",
      value: contract.nyl_representative_name || "Not available",
      confidence: 85,
    },
    {
      label: "Total Amount at Risk",
      value: contract.total_amount_at_risk_desc || "Not specified",
      confidence: 80,
    },
    {
      label: "Premium Percentage",
      value: contract.premium_percentage
        ? `${contract.premium_percentage}%`
        : "Not available",
      confidence: 90,
    },
    {
      label: "Admin Fee Percentage",
      value: contract.admin_fee_percentage
    ...
    {
      label: "Penalty Basis",
      value: contract.penalty_basis || "Not available",
      confidence: 100,
    },
    {
      label: "Signature Status",
      value: contract.signature_status || "UNSIGNED",
      confidence: 92,
    },
    {
      label: "Signature Date",
      value: contract.signature_date || "Not available",
      confidence: 88,
    },
    {
      label: "Total Pages",
      value: contract.contract_pages_total?.toString() || "0",
      confidence: 100,
    },
    {
      label: "Total PGs",
      value: contract.total_pgs?.toString() || "0",
      confidence: 95,
    },
    {
      label: "Standard Count",
      value: contract.standard_count?.toString() || "0",
      confidence: 90,
    },
    {
      label: "Non-Standard Count",
      value: contract.non_standard_count?.toString() || "0",
      confidence: 90,
    },
  ];
  ...
  return {
    ...
    processedAt: contract.extraction_metadata?.server_timestamp
      ? new Date(contract.extraction_metadata.server_timestamp).toLocaleString(
          "en-US",
          {
            month: "2-digit",
            day: "2-digit",
    ...
    metrics: {
      ...
      extractionLabel:
        confidence >= 80 ? "Good" : confidence >= 60 ? "Medium" : "Low",
      completeness:
        contract.total_pgs > 0
          ? Math.round((contract.standard_count / contract.total_pgs) * 100)
          : 0,
      completenessLabel:
...
export function mapContractToUploadedFile(
  ...
  return {
    ...
    name: `${contract.client_name || "Unknown"}.pdf`,
